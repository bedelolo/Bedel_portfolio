export const translations = {
    fr: {
        nav: {
            home: 'Accueil',
            about: 'À propos',
            skills: 'Compétences',
            experience: 'Expérience',
            education: 'Formation',
            projects: 'Projets',
            contact: 'Contact'
        },
        hero: {
            greeting: 'Bonjour, je suis',
            title: 'Développeur Web et Data Analyste',
            description: 'Je conçois des applications web modernes et transforme les données en insights stratégiques. Passionné par le développement full-stack (Django, Laravel) et l\'analyse de données (Python, SQL, Power BI), je crée des solutions digitales qui allient performance technique et intelligence décisionnelle.',
            viewProjects: 'Voir mes projets',
            contact: 'Me contacter'
        },
        about: {
            title: 'À propos de moi',
            p1: 'Je suis un développeur web et data analyste passionné par la création de solutions digitales innovantes. Mon parcours combine le développement d\'applications web robustes et l\'analyse de données pour créer des outils qui transforment les données en décisions stratégiques.',
            p2: 'Côté développement, je maîtrise Django, Laravel, JavaScript et les technologies modernes pour créer des applications web performantes. Côté data, j\'exploite Python, SQL, Power BI et Excel pour extraire des insights pertinents et créer des dashboards interactifs qui guident la prise de décision.',
            stats: {
                webDev: 'Développement Web',
                dataManagement: 'Gestion de Données',
                strategicAnalysis: 'Analyse Stratégique',
                fullStack: 'Solutions Full-Stack'
            }
        },
        skills: {
            title: 'Compétences',
            items: {
                excel: 'TCD, fonctions avancées, tableaux dynamiques',
                sql: 'Requêtes complexes, jointures, bases relationnelles',
                python: 'Pandas, Matplotlib, Seaborn, Jupyter',
                django: 'API REST robustes et backend Python',
                laravel: 'Développement web PHP et applications modernes',
                javascript: 'Interactivité et logique frontend moderne',
                htmlcss: 'Structure sémantique et design responsive',
                tailwind: 'Design rapide et composants utilitaires',
                powerbi: 'Dashboards dynamiques et interactifs',
                storytelling: 'Communication claire des insights',
                git: 'Versionning et collaboration'
            }
        },
        experience: {
            title: 'Expériences professionnelles',
            items: [
                {
                    title: 'Développeur Web et Data Analyste',
                    company: 'AYELTECH',
                    period: 'Novembre 2025 – Aujourd\'hui',
                    description: [
                        'Création de tableaux de bord interactifs avec Power BI pour le suivi des KPIs.',
                        'Développement de sites web et applications web avec Laravel et Django.',
                        'Conception de solutions full-stack alliant développement web et analyse de données.',
                        'Optimisation des processus métier grâce à l\'automatisation et la visualisation de données.'
                    ]
                },
                {
                    title: 'Data Analyst',
                    company: 'GrowUp-AI',
                    period: 'Septembre 2025 – Novembre 2025',
                    description: [
                        'Conception et animation de sessions de formation en Python pour l\'analyse de données.',
                        'Initiation aux bibliothèques Pandas, NumPy, Matplotlib et Seaborn.',
                        'Encadrement d\'apprenants sur des mini-projets pratiques.',
                        'Création de supports pédagogiques clairs et accessibles.'
                    ]
                },
                {
                    title: 'Stagiaire Data Analyst',
                    company: 'Loterie Nationale du Bénin',
                    period: 'Juillet 2025 – Septembre 2025',
                    description: [
                        'Analyse des données opérationnelles.',
                        'Création de rapports interactifs Power BI.',
                        'Optimisation des tableaux de bord Excel pour la prise de décision.'
                    ]
                }
            ]
        },
        education: {
            title: 'Formations',
            items: [
                {
                    degree: 'Licence 3 en Informatique de Gestion',
                    school: 'Institut Cerco',
                    period: '2023 – 2026',
                    desc: 'Spécialisation en analyse de données. Projets orientés data science et business intelligence.'
                },
                {
                    degree: 'Formation continue – Data Analytics',
                    school: 'Laboratoire d\'Intelligence Artificielle - Bénin Excellence',
                    period: '2024 – 2025',
                    desc: 'Modules : Python (bases, POO, Numpy, Pandas, Matplotlib, Seaborn), Machine Learning et Data Storytelling appliqués à des cas réels.'
                }
            ]
        },
        projects: {
            title: 'Projets réalisés',
            items: [
                {
                    title: 'Étude des performances commerciales',
                    desc: 'Tableau de bord Excel pour analyser les performances par région, produit et période. Outil utilisé pour la prise de décision en temps réel.'
                },
                {
                    title: 'Dashboard RH – Visualisation',
                    desc: 'Suivi des indicateurs clés (salaires, âge, satisfaction) et analyse du personnel par genre, département et rôle avec Power BI.'
                },
                {
                    title: 'Gestion Employés - Django',
                    desc: 'Application web pour suivre les employés, gérer les listes et informations. Interface responsive avec Tailwind CSS.'
                },
                {
                    title: 'Optimisation Services Publics',
                    desc: 'Analyse Data-Driven de la performance des services administratifs. Dashboard interactif pour le pilotage stratégique.'
                },
                {
                    title: 'Gestion Congés & Permissions',
                    desc: 'Application complète pour gérer les demandes de congés. Authentification, rôles (Admin/Employé) et export PDF.'
                },
                {
                    title: 'LLM Consensus System',
                    desc: 'Système pour réduire les erreurs des LLMs via consensus. Orchestration performante en Rust et exécution Python.'
                }
            ]
        },
        contact: {
            title: 'Contactez-moi',
            description: 'Je suis ouvert à toute opportunité de collaboration, de mission freelance ou d\'emploi en développement web et data analytics. N\'hésitez pas à me contacter !',
            email: 'Email',
            whatsapp: 'WhatsApp',
            sendEmail: 'Envoyer un Email',
            downloadCV: 'Télécharger mon CV'
        },
        footer: {
            copyright: 'Portfolio Data Analyste'
        }
    },
    en: {
        nav: {
            home: 'Home',
            about: 'About',
            skills: 'Skills',
            experience: 'Experience',
            education: 'Education',
            projects: 'Projects',
            contact: 'Contact'
        },
        hero: {
            greeting: 'Hello, I\'m',
            title: 'Web Developer and Data Analyst',
            description: 'I design modern web applications and transform data into strategic insights. Passionate about full-stack development (Django, Laravel) and data analysis (Python, SQL, Power BI), I create digital solutions that combine technical performance and decision intelligence.',
            viewProjects: 'View my projects',
            contact: 'Contact me'
        },
        about: {
            title: 'About me',
            p1: 'I am a web developer and data analyst passionate about creating innovative digital solutions. My background combines robust web application development and data analysis to create tools that transform data into strategic decisions.',
            p2: 'On the development side, I master Django, Laravel, JavaScript and modern technologies to create high-performance web applications. On the data side, I leverage Python, SQL, Power BI and Excel to extract relevant insights and create interactive dashboards that guide decision-making.',
            stats: {
                webDev: 'Web Development',
                dataManagement: 'Data Management',
                strategicAnalysis: 'Strategic Analysis',
                fullStack: 'Full-Stack Solutions'
            }
        },
        skills: {
            title: 'Skills',
            items: {
                excel: 'Pivot tables, advanced functions, dynamic tables',
                sql: 'Complex queries, joins, relational databases',
                python: 'Pandas, Matplotlib, Seaborn, Jupyter',
                django: 'Robust REST APIs and Python backend',
                laravel: 'PHP web development and modern applications',
                javascript: 'Interactivity and modern frontend logic',
                htmlcss: 'Semantic structure and responsive design',
                tailwind: 'Rapid design and utility components',
                powerbi: 'Dynamic and interactive dashboards',
                storytelling: 'Clear communication of insights',
                git: 'Version control and collaboration'
            }
        },
        experience: {
            title: 'Professional Experience',
            items: [
                {
                    title: 'Web Developer and Data Analyst',
                    company: 'AYELTECH',
                    period: 'November 2025 – Present',
                    description: [
                        'Creating interactive dashboards with Power BI for KPI tracking.',
                        'Developing websites and web applications with Laravel and Django.',
                        'Designing full-stack solutions combining web development and data analysis.',
                        'Optimizing business processes through automation and data visualization.'
                    ]
                },
                {
                    title: 'Data Analyst',
                    company: 'GrowUp-AI',
                    period: 'September 2025 – November 2025',
                    description: [
                        'Designing and conducting Python training sessions for data analysis.',
                        'Introduction to Pandas, NumPy, Matplotlib and Seaborn libraries.',
                        'Mentoring learners on practical mini-projects.',
                        'Creating clear and accessible educational materials.'
                    ]
                },
                {
                    title: 'Data Analyst Intern',
                    company: 'Loterie Nationale du Bénin',
                    period: 'July 2025 – September 2025',
                    description: [
                        'Analyzing operational data.',
                        'Creating interactive Power BI reports.',
                        'Optimizing Excel dashboards for decision-making.'
                    ]
                }
            ]
        },
        education: {
            title: 'Education',
            items: [
                {
                    degree: 'Bachelor\'s Degree in Management Information Systems',
                    school: 'Institut Cerco',
                    period: '2023 – 2026',
                    desc: 'Specialization in data analysis. Projects focused on data science and business intelligence.'
                },
                {
                    degree: 'Continuing Education – Data Analytics',
                    school: 'Artificial Intelligence Laboratory - Bénin Excellence',
                    period: '2024 – 2025',
                    desc: 'Modules: Python (basics, OOP, Numpy, Pandas, Matplotlib, Seaborn), Machine Learning and Data Storytelling applied to real cases.'
                }
            ]
        },
        projects: {
            title: 'Projects',
            items: [
                {
                    title: 'Sales Performance Analysis',
                    desc: 'Excel dashboard to analyze performance by region, product and period. Tool used for real-time decision making.'
                },
                {
                    title: 'HR Dashboard – Visualization',
                    desc: 'Tracking key indicators (salaries, age, satisfaction) and personnel analysis by gender, department and role with Power BI.'
                },
                {
                    title: 'Employee Management - Django',
                    desc: 'Web application to track employees, manage lists and information. Responsive interface with Tailwind CSS.'
                },
                {
                    title: 'Public Services Optimization',
                    desc: 'Data-Driven analysis of administrative services performance. Interactive dashboard for strategic management.'
                },
                {
                    title: 'Leave & Permissions Management',
                    desc: 'Complete application to manage leave requests. Authentication, roles (Admin/Employee) and PDF export.'
                },
                {
                    title: 'LLM Consensus System',
                    desc: 'System to reduce LLM errors through consensus. High-performance orchestration in Rust and Python execution.'
                }
            ]
        },
        contact: {
            title: 'Contact me',
            description: 'I am open to any collaboration opportunity, freelance mission or job in web development and data analytics. Feel free to contact me!',
            email: 'Email',
            whatsapp: 'WhatsApp',
            sendEmail: 'Send an Email',
            downloadCV: 'Download my CV'
        },
        footer: {
            copyright: 'Data Analyst Portfolio'
        }
    }
};
